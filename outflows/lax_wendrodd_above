...
z_d = 1.0  # полутолщина диска в единицах H
beta = 10  # плазменный параметр
Bz = 1.0  # безразмерная компонента Bz
eps = 100  # параметр искусственной вязкости
...
rho0 = 2.5e-10  # плотность в экваториальной плоскости, г/см^3
T0 = 240  # температура в экваториальной плоскости, К
...
def rho_T_profile(z):
    z_norm = z / H
    T = T0 * z_norm**(-9/4)  
    rho = rho0 * np.exp(-0.5 * z_norm**2)
    return rho, T

# завивимость безразмерного давления от безразмерной плотности и температуры
def p(rho, T):
    return rho * T  # Ideal gas law (assuming constant mean molecular weight)
...
def SetIC():
    global t, u_n, pv, rho_n
    
    t = 0.0
    for i in range(Ngs, Ntot - Ngs):
        z = zs[i - Ngs]
        rho_n[i], T = rho_T_profile(z*H) # без учета фиктивных ячеек: i=[Ngs, Ntot-Ngs) = [1, Ntot-2]
...
def F2(u, i):
    rho_i, T_i = rho_T_profile(zs[i-Ngs]*H)
    # = rho*v_z^2 + p + B_phi^2/beta
    return (u[2][i])**2 / u[0][i] + p(rho_i, T_i) + (u[3][i])**2 / beta
